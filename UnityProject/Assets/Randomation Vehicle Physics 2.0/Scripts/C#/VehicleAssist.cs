...
public class VehicleAssist : MonoBehaviour
{
	...
	public int driftSpinExponent = 1;
	...
	void ApplySpinAssist()
	{
		...
		rb.AddRelativeTorque(new Vector3(0,
			(targetTurnSpeed - vp.localAngularVel.y) * driftSpinAssist * driftSpinCurve.Evaluate(Mathf.Abs(F.PowNatural(vp.localVelocity.x, driftSpinExponent))) * groundedFactor//driftSpinCurve.Evaluate(Mathf.Abs(F.PowNatural(vp.localAngularVel.y,driftSpinExponent))) * -Mathf.Sign(vp.localAngularVel.y)
