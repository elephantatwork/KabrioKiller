...
public class HoverWheel : MonoBehaviour
{
	...
	void ApplyFloat()
	{
		if (grounded)
		{
			...
			rb.AddForceAtPosition(upDir * floatForce * (F.PowNatural(floatForceCurve.Evaluate(1 - compression), Mathf.Max(1, floatExponent)) - floatDampening * Mathf.Clamp(travelVel, -1, 1))
				, tr.position
