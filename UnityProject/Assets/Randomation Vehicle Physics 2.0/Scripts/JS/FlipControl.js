...
public class FlipControl extends MonoBehaviour
{
	...
	function ApplyRotationCorrection()
	{
		...
		rb.AddRelativeTorque(new Vector3(
			vp.pitchInput * flipPower.x == 0 ? actualForwardDot * (1 - Mathf.Abs(actualRightDot)) * rotationCorrection.x - vp.localAngularVel.x * F.PowNatural(actualUpDot, 2) * 10 : 0,
			vp.yawInput * flipPower.y == 0 && vp.sqrVelMag > 10 ? Vector3.Dot(vp.forwardDir, velDir * Vector3.right) * Mathf.Abs(actualUpDot) * rotationCorrection.y - vp.localAngularVel.y * F.PowNatural(actualUpDot, 2) * 10 : 0,
			vp.rollInput * flipPower.z == 0 ? -actualRightDot * (1 - Mathf.Abs(actualForwardDot)) * rotationCorrection.z - vp.localAngularVel.z * F.PowNatural(actualUpDot, 2) * 10 : 0
