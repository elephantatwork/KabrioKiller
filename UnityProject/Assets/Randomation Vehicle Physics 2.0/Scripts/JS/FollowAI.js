...
public class FollowAI extends MonoBehaviour
{
	...
	function FixedUpdate()
	{
		if (target)
		{
			...
			close = (tr.position - target.position).sqrMagnitude <= F.PowNatural(followDistance, 2) && !targetIsWaypoint;
			...
			vp.SetSteer(reverseTime == 0 ? Mathf.Abs(F.PowNatural(steerDot, (tr.position - target.position).sqrMagnitude > 20 ? 1 : 2)) * Mathf.Sign(steerDot) : -Mathf.Sign(steerDot) * (close ? 0 : 1));
